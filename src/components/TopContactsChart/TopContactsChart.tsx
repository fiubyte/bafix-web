...
const TopContactsChart = ({initialDate, finalDate}: TopContactsChartProps) => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`${config.apiUrl}/services/metric/top_contacts/?start_date=${initialDate.format().slice(0, 19)}&end_date=${finalDate.format().slice(0, 19)}`, {headers: {"Authorization": `Bearer ${localStorage.getItem(config.LOCAL_STORAGE_JWT_KEY)}`}}).then(r => {
      setData(r.data);
      setLoading(false);
    }).catch(e => {
      console.error(e);
  ...
  const renderUser = (user: TopContactsData) => {
    return (
      <Box className={"TopContactsChart-user"}>
        <img src={user.photo_url} alt={`${user.user_name} ${user.user_surame}`} className={"TopContactsChart-image"}/>
